...
const retrystrategy = require('@magaya/socket-tunnel-node/lib/retrystrategy');
...
module.exports = (apiCredentials) => {
    ...
    async function getCarrierByAir(airScac) {
        ...
        if (!result) {
            result = await findTransactionByCondition(
                dbxEntityCarrierActiveList,
                (ca) => ca.CarrierTypeCode === airMethod && ca.AirlineCodeNumber === airScac,
            );
        }
        return result;
    }
    ...
    async function findPort(code) {
        ...
        const found = await getDataByCondition(portList, (current) => current.Code === code);
        return found;
    }
    async function portValidation(jsonData, ent) {
        ...
        const entity = jsonData[ent] && {
            [`${entBase}`]: {
                '@': {
                    Code: (found && found.Code) || jsonData[`${ent}`].Code || jsonData[`${ent}`],
                },
                Country: {
                    '@1': {
                ...
                Name: (found && found.Name) || jsonData[`${ent}`].Code || jsonData[`${ent}`],
                Subdivision: (found && found.Subdivision) || jsonData[`${ent}`].Code || jsonData[`${ent}`],
            },
        };
        return entity;
    }
                ...
                Manufaturer: {
                    ...
                    Address: {
                        Street:
                            ...
                            item.Manufacturer.Address.Street,
                        City:
                            item.Manufacturer && item.Manufacturer.Address && item.Manufacturer.Address.City,
                        State:
                            item.Manufacturer && item.Manufacturer.Address && item.Manufacturer.Address.State,
                        ZipCode:
                            item.Manufacturer &&
                            item.Manufacturer.Address &&
                            item.Manufacturer.Address.ZipCode,
                        Country: {
                            '@': {
                                Code:
                                    item.Manufacturer &&
                                    item.Manufacturer.Address &&
                                    item.Manufacturer.Address.Country,
                            },
                            '#':
                                item.Manufacturer &&
                                item.Manufacturer.Address &&
    ...
    async function getDataEntities(jsonData) {
        const motCode = jsonData && jsonData.Master && jsonData.Master.Mot && jsonData.Master.Mot.Code;
        const scac = jsonData && jsonData.Master && jsonData.Master.Scac;
        const currentCarrier =
            ((motCode === '40' || motCode === '41') && (await getCarrierByAir(scac))) ||
        ...
        const carrier = currentCarrier && {
            ...
            CarrierAddress: {
                ...
                City: currentCarrier && currentCarrier.Address && currentCarrier.Address.City,
                State: currentCarrier && currentCarrier.Address && currentCarrier.Address.State,
                ZipCode: currentCarrier && currentCarrier.Address && currentCarrier.Address.ZipCode,
                Country: {
                    '@': {
                        Code: currentCarrier && currentCarrier.Address && currentCarrier.Address.Country,
                    },
                    '#': currentCarrier && currentCarrier.Address && currentCarrier.Address.Country,
                },
            },
        };
        ...
        for (const house of dataHouses) {
            const data = {
                ...
                ModeOfTransportation: {
                    ...
                    Description:
                        (jsonData.Master && jsonData.Master.Mot && jsonData.Master.Mot.Description) ||
    ...
    async function getDataOnlyHouse(jsonData, dataHouses) {
        ...
        const dataItem = jsonData.Items && jsonData.Items.length > 0 && (await getDataItems(jsonData));
        for (const house of dataHouses) {
            const data = {
                '@': {
                    Type: 'SH',
    ...
    async function getDataHouses(jsonData, dataHouses) {
        const motCode = jsonData && jsonData.Master && jsonData.Master.Mot && jsonData.Master.Mot.Code;
        const infoShip = ((motCode === '40' || motCode === '41') && 'AirShipment') || 'OceanShipment';
        const data = {
    ...
    async function getDataGeneral(jsonData) {
        ...
        const destinationPort = await portValidation(jsonData.Master, 'PortOfUnlading');
        ...
        const shipInfo = ((motCode === '40' || motCode === '41') && {
            AirShipmentInfo: {
                AirWayBillNumber: jsonData.Master && jsonData.Master.WayBillNumber,
                FlightNumber: (jsonData.Master && jsonData.Master.VesselFlightNumb) || '',
            },
        }) || {
            OceanShipmentInfo: {
                BillOfLadingNumber: jsonData.Master && jsonData.Master.WayBillNumber,
                VoyageIdentification: (jsonData.Master && jsonData.Master.VesselFlightNumb) || '',
        ...
        const data = {
            ...
            Number: (jsonData.Master && jsonData.Master.RefNumber) || jsonData.Master.WayBillNumber,
            ModeOfTransportation: {
                '@': {
                    Code: motCode || '11',
                },
    ...
    async function getDataShipment(jsonData) {
        ...
        const wayNumber = jsonData && jsonData.Master && jsonData.Master.WayBillNumber;
        ...
        // if (jsonData.Houses && jsonData.Houses.length !== 1) {
        ...
        //     };
        // }
        // const dataHouse = await getDataOnlyHouses(jsonData, listHouses);

        // return { ...dataGeneral, ...dataHouse };
    }
    ...
    async function transformToXml({ jsonData }, type, transactionId) {
        ...
        const motCode = jsonData && jsonData.Master && jsonData.Master.Mot && jsonData.Master.Mot.Code;
        const rootType = ((motCode === '40' || motCode === '41') && 'AirShipment') || 'OceanShipment';
        const root = rootType;
    ...
    const createShipment = async (jsonData, transactionId) => {
        ...
        // const flags = 128;
        let errorMsg = '';
        await soapApiClient.initSoapClient(apiCredentials.Username, apiCredentials.UserPassword);
        const response = await soapApiClient.setTransaction({ xml: xmlShipment, type }, apiCredentials);
